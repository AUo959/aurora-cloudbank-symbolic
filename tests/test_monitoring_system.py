...
class TestMonitoringSystem:
    ...
    def test_initialization(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            if monitoring.behavior_monitor is None:
                raise AssertionError("MonitoringSystem must wire behavior_monitor")
            if monitoring.behavior_monitor.current_metrics != {}:
                raise AssertionError("Expected behavior_monitor.current_metrics to start empty")
            if monitoring.drift_detector is None:
                raise AssertionError("MonitoringSystem must wire drift_detector")
            if monitoring.drift_detector.alerts_path != Path(tmpdir) / "drift_alerts.jsonl":
                raise AssertionError("Expected drift_detector.alerts_path to match storage_dir")
            if monitoring.ethics_engine is None:
                raise AssertionError("MonitoringSystem must wire ethics_engine")
            if monitoring.ethics_engine.violations_path != Path(tmpdir) / "ethics_violations.jsonl":
                raise AssertionError("Expected ethics_engine.violations_path to match storage_dir")
            if monitoring.audit_logger is None:
                raise AssertionError("MonitoringSystem must wire audit_logger")
            if monitoring.audit_logger.storage_path != Path(tmpdir) / "audit_log.jsonl":
                raise AssertionError("Expected audit_logger.storage_path to match storage_dir")
