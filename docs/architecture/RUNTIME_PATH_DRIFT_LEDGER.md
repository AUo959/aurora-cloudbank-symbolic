...
| `src/api/mesh_api.js` as production mesh authority | legacy, test-only | The Express router is mounted in `tests/node/mesh_api_activation.test.js`; the API inventory and runtime-topology authority both identify the Python mesh runtime as canonical. No production mount exists. | Retain as a legacy Node compatibility test surface. Do not promote it to production; revisit only if a named deployment owner and migration contract emerge. |
...
| `docs/api/API_CATALOG.json`, `docs/api/api_schema.json`, and `docs/api/API_CATALOG.md` | generated snapshot | Regenerated on 2026-07-13 with machine-readable generation time and source-commit metadata. | Use `docs/api/api_surface_inventory.json` for governance ownership; use the snapshots for route detail only after comparing their `source_commit` with the intended runtime revision. |
| `scripts/generate_api_catalog.py` output location | **resolved** | Since commit `53449985`, the default output directory is repo-root `docs/api/` regardless of current working directory. The generator now assembles security and monitoring routes with process-local fixtures and records source metadata. | Run `python scripts/generate_api_catalog.py` from any directory in the checkout; use `--output-dir` only for an intentional preview. |
