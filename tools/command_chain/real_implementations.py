...
class RealCommandImplementations:
    ...
    def git_commit(self, message: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create commit (output ignored - we get hash separately)
    ...
    def format_code(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Run black
            try:
                # Run black (output ignored in quiet mode)
                self._run_command(['python', '-m', 'black', '.', '--quiet'])
            ...
            # Run isort
            try:
                # Run isort (output ignored in quiet mode)
                self._run_command(['python', '-m', 'isort', '.', '--quiet'])
