...
class BranchManager:
    ...
    def cleanup_stale_branches(self, max_age_days: int = 90, categories: List[str] = None) -> Dict:
        ...
        for branch in branches:
            should_delete = (
                branch.category in categories and
                branch.is_merged and
                branch.days_old > max_age_days and
                branch.name not in ['main', 'develop', 'master']
            )
...
def main():
    ...
    parser.add_argument('--execute', action='store_true', help='Execute actual cleanup (overrides dry-run)')
    parser.add_argument('--max-age', type=int, default=90, help='Maximum age in days for cleanup')
    parser.add_argument('--categories', nargs='+', default=['feature', 'dependency', 'security'],
    ...
    if args.analyze:
        ...
        # Save report to file
        with open('branch_management_report.md', 'w') as f:
            f.write(report)
