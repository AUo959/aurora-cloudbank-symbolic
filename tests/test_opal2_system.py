...
from ..plugin_system import PluginInterface, PluginSystem, PluginType, RendererPlugin
from ..quantum_renderer import (
    QuantumRenderer,
    QuantumState,
    RenderContext,
    RenderResult,
...
class TestGlyphCache:
    ...
    async def test_cache_set_get_async(self):
        ...
        with patch.object(self.glyph_cache, "set_async", return_value=True) as mock_set:
            with patch.object(
                self.glyph_cache, "get_async", return_value=self.test_data
            ) as mock_get:

                # Test set
                set_result = await self.glyph_cache.set_async(
                    self.test_key, self.test_data
                )
    ...
    async def test_cache_clear_async(self):
        ...
        with patch.object(
            self.glyph_cache, "clear_async", return_value=5
        ) as mock_clear:
    ...
    async def test_cache_stats(self):
        ...
        with patch.object(
            self.glyph_cache,
            "get_stats",
            return_value={"total_items": 10, "hit_rate": 0.85, "miss_rate": 0.15},
        ) as mock_stats:
...
class TestConfigurationManager:
    ...
    def test_config_validation(self):
        ...
        invalid_config = {
            ...
            "canvas": {"width": 50, "height": 600},  # Too small
            "quantum": {
    ...
    def test_config_value_operations(self):
        ...
        # Test getting values
        default_engine = self.config_manager.get_config_value(
            "opal2_graphics", "renderer.default_engine"
        )
        assert default_engine == "webgl"

        # Test setting values
        result = self.config_manager.set_config_value(
            "opal2_graphics", "renderer.default_engine", "canvas"
        )
        assert result is True

        updated_engine = self.config_manager.get_config_value(
            "opal2_graphics", "renderer.default_engine"
        )
