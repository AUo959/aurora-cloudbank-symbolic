...
This script is intentionally conservative. It reads local queue files and emits a
small metrics packet or Markdown report. It does not mutate queue state, create
claims, call GitHub, edit control-plane files, or touch runtime code.
...
def item_status(item: dict[str, Any]) -> str:
    # Current live queue uses status. Future schema may use state.
