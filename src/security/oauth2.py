...
if not SECRET_KEY:
    raise RuntimeError(
        "JWT_SECRET_KEY environment variable must be set. " "Generate a strong random key (e.g., openssl rand -hex 32)"
    )
...
class OAuth2Handler:
    ...
    @classmethod
    def authenticate_user(cls, username: str, password: str, user_db: Dict[str, UserInDB]) -> Optional[UserInDB]:
        """
...
async def get_current_user(token: str = Depends(oauth2_scheme)) -> User:
    ...
    user = User(username=token_data.username, role=Role(token_data.role) if token_data.role else Role.OBSERVER)
    ...
    def decorator(func):
        ...
        async def wrapper(*args, current_user: User = Depends(get_current_active_user), **kwargs):
            if current_user.role != role:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN, detail=f"Insufficient role. Required: {role.value}"
                )
            return await func(*args, current_user=current_user, **kwargs)
