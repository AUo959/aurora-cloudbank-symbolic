...
def main():
    ...
    if Path("scripts/gitwiz_enhanced.py").exists():
        test_command([sys.executable, "-c", "from scripts.gitwiz_enhanced import EnhancedGITWiz; print('✅ Enhanced GITWiz import successful')"], "Enhanced GITWiz Import")
    
    # Test 3: Dependency Updater
    if Path("scripts/gitwiz_dependency_updater.py").exists():
        test_command([sys.executable, "scripts/gitwiz_dependency_updater.py", "--help"], "Dependency Updater Help")
        test_command([sys.executable, "scripts/gitwiz_dependency_updater.py", "--scan"], "Dependency Scanner")
    
    # Test 4: Workflow Orchestrator
    if Path("scripts/gitwiz_workflow_orchestrator.py").exists():
        test_command([sys.executable, "scripts/gitwiz_workflow_orchestrator.py", "--help"], "Workflow Orchestrator Help")
    ...
    if Path("hdeplusplus.py").exists():
        test_command([sys.executable, "hdeplusplus.py", "recommend", "--context", '{"weights": {"logic":3}}'], "HDE++ Recommendation")
