...
class MaintenanceScheduler:
    ...
    def cleanup_python_cache(self) -> Dict:
        ...
        try:
            ...
            # Remove .pyc files
            if pyc_count > 0:
                subprocess.run(['find', '.', '-name', '*.pyc', '-delete'], cwd=self.repo_path)
            ...
            if pycache_count > 0:
                subprocess.run(['find', '.', '-name', '__pycache__', '-type', 'd', '-exec', 'rm', '-rf', '{}', '+'], cwd=self.repo_path)
            ...
            self.logger.info(f"Python cache cleanup complete: {pyc_count} .pyc files, {pycache_count} __pycache__ dirs")
        ...
        try:
            ...
            for pattern in temp_patterns:
                ...
                find_result = subprocess.run(
                    ['find', '.', '-name', pattern, '-type', 'f', '!', '-path', './.venv/*', '!', '-path', './.git/*'],
                    capture_output=True, text=True, cwd=self.repo_path
                )
            ...
            self.logger.info(f"Temporary file cleanup complete: {removed_count} files removed")
    ...
    def run_health_check(self) -> Dict:
        ...
        try:
            ...
            if health_script.exists():
                ...
                # Basic health check
                size_result = subprocess.run(['du', '-sm', '.'], capture_output=True, text=True, cwd=self.repo_path)
                files_result = subprocess.run(['find', '.', '-type', 'f'], capture_output=True, text=True, cwd=self.repo_path)
    ...
    def cleanup_stale_branches(self) -> Dict:
        ...
        try:
            ...
            if branch_script.exists():
                ...
                result['details'] = {
                    'analysis_output': branch_result.stdout[:1000] + '...' if len(branch_result.stdout) > 1000 else branch_result.stdout
                }
            else:
                # Basic branch count
                branch_result = subprocess.run(
            ...
            if (self.repo_path / 'package.json').exists():
                ...
                if audit_result.stdout:
                    try:
                        ...
                        result['details']['node_vulnerabilities'] = audit_data.get('metadata', {}).get('vulnerabilities', {}).get('total', 0)
                    except json.JSONDecodeError:
