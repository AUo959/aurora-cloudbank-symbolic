...
- [subroutines-enhanced](#subroutines-enhanced)
- [synergy](#synergy)
...
**Summary:** CASK project risk assessment
...
**Summary:** CASK technical specifications per component
...
**Summary:** CASK comparison against state of the art
...
**Summary:** CASK component topology metadata
...
**Summary:** Coordinate Collaboration
...
### `GET /api/crew/division/{division}`

**Summary:** Get Agents By Division Endpoint
...
### `GET /api/crew/role/{role}`

**Summary:** Get Agents By Role Endpoint
...
### `GET /api/crew/{agent_surname}/capabilities`
...
### `POST /api/crew/{agent_surname}/process`
...
### `GET /api/crew/{agent_surname}/status`
...
### `POST /gumas/ethics_check`

**Summary:** Unified Ethics Check
...
- `operation_type` (query) - Optional
  - Filter by operation type
- `failures_only` (query) - Optional
...
- `time_window` (query) - Optional
  - Time window in seconds to summarize metrics
- `context_tag` (query) - Optional
  - DLP context tag for lineage tracking
...
**Summary:** Test Telemetry Operation
...
- `agent_id` (query) - Optional
  - Filter by agent ID
- `level` (query) - Optional
  - Filter by alert level (info/warning/critical)
- `limit` (query) - Optional
...
Establish a new baseline for an agent/metric combination
...
Get current baseline configurations for all monitored metrics
...
**Summary:** Health check
Check drift monitoring health status
...
### `GET /checkpoint/agent/{agent_id}/history`
...
### `POST /checkpoint/{checkpoint_id}/rollback`

**Summary:** Rollback Checkpoint
...
### `GET /api/l2-agents/activation-phrases`
...
### `GET /api/l2-agents/agent/{agent_id}`
...
### `GET /api/l2-agents/constellation`
...
### `POST /api/l2-agents/disconnect/{agent_id}`
...
### `GET /playground/results/{session_id}`
...
### `GET /playground/share/{short_code}`

**Summary:** Fetch Shared

**Parameters:**

- `short_code` (path) - ✅ Required
...
**Summary:** List Example Scenarios
...
**Summary:** Send Cross Layer Message
...
### `GET /api/sensors/external/astronomical`
...
### `GET /api/sensors/external/communications`
...
### `GET /api/sensors/external/deep-space`
...
### `GET /api/sensors/external/proximity`
...
### `GET /api/sensors/external/salvage`
...
### `GET /api/sensors/fusion/certification`
...
### `GET /api/sensors/fusion/forecasts`
...
### `GET /api/sensors/fusion/forecasts/{anomaly_type}`

**Summary:** <Lambda>

**Parameters:**

- `anomaly_type` (path) - ✅ Required
...
### `GET /api/sensors/fusion/oscillation-health`
...
### `GET /api/sensors/fusion/resonance`
...
### `GET /api/sensors/health/performance`
...
### `GET /api/sensors/internal/biometrics`
...
### `GET /api/sensors/internal/environmental`
...
### `GET /api/sensors/internal/operational`
...
### `GET /api/sensors/internal/structural`
...
### `GET /api/sensors/observatory/physical/boundary`
...
### `GET /api/sensors/observatory/physical/containment`
...
### `GET /api/sensors/observatory/physical/earth-relay`
...
### `GET /api/sensors/observatory/physical/fidelity`
...
### `GET /api/sensors/observatory/physical/reality-anchor`
...
### `GET /api/sensors/observatory/symbolic/concept-resonance`
...
### `GET /api/sensors/observatory/symbolic/drift-presignature`
...
### `GET /api/sensors/observatory/symbolic/ethical-signal`
...
### `GET /api/sensors/observatory/symbolic/ethical-signal/{entity_id}`
...
### `GET /api/sensors/observatory/symbolic/integration-depth`
...
### `POST /subroutines/anomaly/detect`
...
### `GET /subroutines/dependencies/health`
...
### `POST /subroutines/ethics/check`

**Summary:** Check Ethics Compliance
...
### `GET /subroutines/ethics/stats`

**Summary:** Get Ethics Stats
...
### `GET /subroutines/integration/validate`

**Summary:** Validate Integrations
...
### `GET /subroutines/knowledge/sync`

**Summary:** Sync Knowledge Bases
...
### `GET /subroutines/performance/profile`
...
### `POST /subroutines/quantum/optimize`

**Summary:** Optimize Circuit
...
### `POST /subroutines/resources/analyze`

**Summary:** Analyze Resources
...
### `POST /subroutines/security/scan`
