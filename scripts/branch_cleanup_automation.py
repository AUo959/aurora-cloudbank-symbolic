...
class BranchCleanupManager:
    ...
    def get_branch_info(self) -> List[Dict]:
        ...
        try:
            ...
            cmd = ["git", "for-each-ref", "--format=%(refname:short)|%(committerdate:iso8601)|%(authorname)|%(subject)", "refs/remotes/origin/"]
    ...
    def generate_cleanup_report(self, categories: Dict[str, List[Dict]]) -> str:
        ...
        for category, branches in categories.items():
            ...
            report.append(f"## {category.replace('_', ' ').title()} ({len(branches)} branches)")
            report.append("")
