...
def get_merged_branches():
    result = subprocess.run(["git", "branch", "-r", "--merged", "origin/main"], capture_output=True, text=True)
...
def delete_remote_branch(branch):
    ...
    if name == "main":
        return
    subprocess.run(["git", "push", remote, f":{name}"])

def archive_branch(branch):
    tag_name = f"archive/{branch.replace('/', '_')}"
    subprocess.run(["git", "tag", tag_name, branch])
