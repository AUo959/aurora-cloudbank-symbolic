"""
Superposition Gate

Combines independently-evaluated Verdicts from heterogeneous safety/ethics
evaluators (or heterogeneous execution backends) into one decision via a
deterministic, hard-veto-first collapse rule -- without requiring the source
evaluators to share an implementation or call each other.

This module is intentionally standalone: it has no knowledge of any specific
caller. `src/monitoring/ethics_gate.py::check_ethics()` is the first
evaluator wired to produce a Verdict and call collapse() here; the Ethics
Field's dimension_evaluators and EthicsAwareQuantumGate remain separate,
independently-reviewable follow-ups. See README.md in this directory for
the design rationale and current integration status.

DLP: context_tag=superposition_gate_init, symbolic_hash=SUPERPOSITION_GATE_v1
"""

__version__ = "0.1.0"

from .core import EmptyVerdictSetError, collapse
from .models import CollapsedVerdict, Verdict, VerdictSeverity

__all__ = [
    "Verdict",
    "VerdictSeverity",
    "CollapsedVerdict",
    "collapse",
    "EmptyVerdictSetError",
]
