...
class MemoryCompressionOptimizer:
    ...
    def __init__(self, dry_run: bool = True):
        ...
        self.deduplication_config = {
            'check_extensions': {'.zip', '.pdf', '.png', '.jpg', '.jpeg', '.md', '.txt'},
    ...
    def analyze_repository(self) -> Dict:
        ...
        for root, dirs, files in os.walk('.'):
            ...
            dirs[:] = [d for d in dirs if not any(ex in str(Path(root) / d) for ex in self.compression_config['exclude_paths'])]
    ...
    def optimize_large_files(self, large_files: List[Dict]) -> Dict:
        ...
        for file_info in large_files:
            ...
            if file_path.suffix.lower() in {'.zip', '.gz', '.bz2', '.xz', '.7z', '.rar'}:
                continue
...
def main():
    ...
    parser.add_argument('--compress-only', action='store_true',
                       help='Only run compression optimization')
    parser.add_argument('--dedupe-only', action='store_true',
                       help='Only run deduplication')
