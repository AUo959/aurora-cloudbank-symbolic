...
def generate_ci_summary():
    ...
    if quality_report:
        if quality_report.get('summary', {}).get('total_issues', 0) > 0:
            recommendations.append("- Run `python scripts/gitwiz_integrated_command.py quality-check --auto-fix` to resolve auto-fixable issues")
    ...
    json_summary = {
        ...
        "overall_status": "PASS" if (
            (quality_report.get('summary', {}).get('total_issues', 0) == 0 if quality_report else True) and
            (len(bandit_report.get('results', [])) == 0 if bandit_report else True)
