...
class ConfigurationManager:
    ...
    def _initialize_default_schemas(self):
        ...
        self.validation_rules["opal2_graphics"] = [
            ...
            ConfigValidationRule("renderer.default_engine", required=True, type_check=str, 
                               allowed_values=["webgl", "canvas", "svg"]),
            ConfigValidationRule("renderer.quantum_enhancement", required=True, type_check=bool),
            ConfigValidationRule("renderer.performance_mode", required=True, type_check=str,
                               allowed_values=["fast", "balanced", "quality"]),
            ConfigValidationRule("canvas.width", required=True, type_check=int, min_value=100, max_value=4096),
            ConfigValidationRule("canvas.height", required=True, type_check=int, min_value=100, max_value=4096),
            ConfigValidationRule("quantum.coherence_factor", required=True, type_check=float, 
                               min_value=0.0, max_value=1.0),
            ConfigValidationRule("quantum.entanglement_strength", required=True, type_check=float,
                               min_value=0.0, max_value=1.0),
            ConfigValidationRule("quantum.superposition_depth", required=True, type_check=int,
        ...
        self.validation_rules["api"] = [
            ...
            ConfigValidationRule("server.port", required=True, type_check=int, min_value=1, max_value=65535),
            ...
            ConfigValidationRule("rate_limiting.requests_per_minute", required=False, type_check=int, min_value=1),
    ...
    def _handle_file_change(self, file_path: str):
        ...
        if path.parent == self.config_dir and path.suffix in ['.yaml', '.yml', '.json', '.toml']:
    ...
    def export_config(self, config_name: str, export_path: str, format_type: ConfigFormat = ConfigFormat.YAML) -> bool:
