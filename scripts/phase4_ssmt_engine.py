...
class SSMTEngine:
    ...
    def run_command(self, command: str, check_return: bool = True) -> Optional[subprocess.CompletedProcess]:
        ...
        try:
            ...
            print("⏱️ Command timed out: %s", command)
            return None
    ...
    def analyze_branch_value(self, branch_name: str) -> Dict:
        ...
        try:
            ...
            print("⚠️ Error analyzing {branch_name}: %s", e)
...
🎯 PROCESSING BATCH: %s", batch_name)
        print("   Strategy: %s", batch_config['strategy'])
        print("   Branches: %s", len(branches))
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            for future in as_completed(analysis_futures):
                ...
                try:
                    ...
                    print("📊 {branch}: %s (Value: {analysis[", analysis['recommendation'])
                except Exception as e:
                    print("❌ Analysis failed for {branch}: %s", e)
        ...
        for branch in branches:
            ...
            if analysis.get("recommendation") in ["high_value", "moderate_value"]:
                ...
                if merge_result["status"] == "success":
                    ...
                    print("⚠️ {branch}: %s", merge_result['status'])
            else:
                print("⏭️ Skipping {branch}: %s", analysis.get('recommendation', 'unknown'))
    ...
    def run_phase4_integration(self):
        ...
        print("   Successful integrations: %s", total_successful)
        print("   Failed integrations: %s", total_failed)
