...
def test_alias_and_folder_high_priority():
    text = "Threadcore symbolic anchor drift vector reflect seal"    result = tag_thread_context(text)    assert result["alias"] == "SymbolicOps"
...
def test_unsorted_low_priority():
    ...
    assert result["alias"] == "Unsorted"    result = tag_thread_context("")    assert result["priority"] == "low"
